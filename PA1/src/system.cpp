...
System::partitionWorstFit()
{
    ...
    for (int i = 0; i < numThread; i++) {
        ...
        float cpu_U_2 = cpuSet[2].utilization();
        float cpu_U_3 = cpuSet[3].utilization();
        if (cpuSet[0].utilization() + threadSet[i].utilization() < 1 && cpu_U_0 <= cpu_U_1 && cpu_U_0 <= cpu_U_2 && cpu_U_0 <= cpu_U_3) { threadSet[i].setUpCPUAffinityMask(0); cpuSet[2].pushThreadToCPU(&threadSet[i]); }
