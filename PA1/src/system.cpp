...
System::partitionBestFit()
{
    ...
    for (int i = 0; i < numThread; i++) {
        ...
        else if (cpuSet[2].utilization() + threadSet[i].utilization() < 1 && cpu_U_0 != 0 && cpu_U_1 != 0 && cpu_U_2 == 0 && cpu_U_3 == 0) { threadSet[i].setUpCPUAffinityMask(2); cpuSet[0].pushThreadToCPU(&threadSet[i]); }
